...
// RUTAS (si mueves archivos, ajusta aquí)
const ROUTES = {
  ausencias: '/ausencias',
  tardanzas: '/tardanzas',
} as const;

// Debe coincidir con el DTO de tu backend
interface ResumenMensual {
  ...
  tiempoTotalTardanzas?: number; // minutos
}

// Helper para minutos → "1h 25m"
...
// Fila reutilizable de estadísticas (sin navegación)
const StatRowBase = ({
  label,
  value,
  caption,
...
// Fila con capacidad de navegación (toda la fila es clickeable)
const StatRowLink = ({
  onPress,
  ...rest
...
export default function TableroScreen() {
  ...
  const fetchResumen = async () => {
    try {
      ...
      if (!API_URL) throw new Error('La URL de la API no está configurada en app.config.js');
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        <View style={styles.statsContainer}>
          ...
          {/* Otras métricas (sin navegación, o añade pantallas si las creas) */}
        ...
        {/* Footer: fecha de inicio laboral (opcional) */}
        {resumen.fechaInicioLaboral ? (
          <View style={styles.footer}>
            <Text style={styles.footerText}>Fecha de inicio laboral</Text>
            <Text style={styles.footerDate}>
              {dayjs(resumen.fechaInicioLaboral).format('DD [de] MMMM [de] YYYY')}
            </Text>
          </View>
        ) : null}
      </ScrollView>
